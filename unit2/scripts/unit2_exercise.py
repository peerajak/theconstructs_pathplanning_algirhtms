...
def search_node_in_list(list_to_search, index_to_search):
  '''
  Return the node in the list whose index is index_to_search or return None if not found
  '''
  for element in list_to_search:
    if element.get('index') == index_to_search:
        return element
  else:
        return None
...
def dijkstra(start_index, goal_index, width, height, costmap, resolution, origin, grid_viz = None):
  ...
  # 0. a Node is a tuple of (position, distance_to_start, parent)
  # 1. put start_index node into open_group
  # 2. get a list of neightbors of start_index
  # 3. 
  #  while goal_index has not been added to open group
  #  for each list element, 
  #      - for each nieghtor create a node
  #      - if the above node never been added to open_group, add it to open group. add distance to start_index, and parent position
  #      - if the above node had already been added to open_group,  if the new distance is less than previous distance,
  #               change the following param value to open group: distance to start_index, and parent position
  #      - if the above node had already been added to open_group,  if the new distance is larger or equal o previous distance, then do nothing
  #      - get the list of neightbors of each eelement and goto loop for
